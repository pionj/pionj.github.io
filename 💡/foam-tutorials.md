...
확장자까지 입력을 하면 안되는건가. 참고로 한글도 인식이 안된다. 빈문자열로 되버린다. 영어만 가능.
...
11. 이미지는 어차피 절대경로든 상대경로든 의미없다. 그냥 기존 마크다운 문법대로 상대경로로 path 지정해놓으면 알아서 변환해준다. 변환 당시 읽기 가능한 경로로 입력해야함. 아래 예시는 전부 안됨.
```
⛔:  
![짐바드로-시간관검사](📦/짐바드로-시간관검사)  
